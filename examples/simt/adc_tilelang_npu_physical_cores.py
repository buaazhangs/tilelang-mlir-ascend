"""ADC distance kernel using physical-core task scheduling for NPUIR.
...
The kernel launches a fixed number of physical NPU cores. Each physical
core handles one or more logical document blocks. For every subspace in
each logical block, the kernel separates the original mixed loop into:
...
def make_adc_kernel(block_M=256, num_subspaces=64,
                    codebook_size=256, num_kernels=56):
    return adc_distance_kernel(
        block_M,
...
def main(n, block_M, num_subspaces, codebook_size, num_kernels):
    if n <= 0 or block_M <= 0:
    ...
    print(f"PASS n={n} block_M={block_M} num_kernels={num_kernels}")
