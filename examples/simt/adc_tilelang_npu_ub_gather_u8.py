"""ADC distance kernel with uint8 codes cast inside the NPUIR kernel.
...
This is based on adc_tilelang_npu_ub_gather.py, but keeps doc codes in uint8
until they are staged into UB. The kernel then casts the transposed UB tile to
int32 before T.gather, which avoids the external PyTorch uint8 -> int32 tensor.
"""
...
def adc_distance_ub_gather_u8_kernel(block_M, num_subspaces=64,
                                     codebook_size=256, dtype="float32",
                                     code_dtype="uint8", num_kernels=56,
                                     subspace_tile=16):
    if code_dtype != "uint8":
    ...
    def adc_func(
        ...
        with T.Kernel(num_kernels, is_npu=True) as (kernel_id, _):
            ...
            CODES_U8_UB = T.alloc_ub((subspace_tile, block_M), code_dtype)
            ...
            for task_id in T.serial(T.ceildiv(num_logic_kernels, num_kernels)):
                ...
                if logic_kernel_id < num_logic_kernels:
                    ...
                    for sg in T.serial(num_subspace_tiles):
                        ...
                        T.transpose(CODES_TILE_U8_UB, CODES_U8_UB, [1, 0])
                        T.vcast(CODES_U8_UB, CODES_I32_UB)
...
def make_adc_kernel(block_M=256, num_subspaces=64,
    ...
    return adc_distance_ub_gather_u8_kernel(
        block_M,
