"""ADC 2D LUT indirect-load kernel with physical-core task scheduling.
...
adc_tilelang_npu.py. Each physical NPU core handles one or more logical
document blocks, then isolates only the 2D LUT access:
...
def make_lut2d_kernel(block_M=256, num_subspaces=64,
                      codebook_size=256, num_kernels=56):
    return adc_lut2d_indirect_kernel(
        block_M,
